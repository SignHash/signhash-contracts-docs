...
export default class Method extends Component {
  ...
  componentDidMount() {
    if (
      this.props.method.type === 'function' &&
  ...
  handleRequest() {
    ...
    calledMethod.call.apply(calledMethod, [
      ...this.state.inputs,
      (err, res) => {
        const results = Array.isArray(res) ? res : [res];
        // format bignumbers
        const outputs = results.map(
          out => (out.toNumber ? `${out.toNumber()}` : `${out}`)
        );
  ...
  handleUpdateParam(e, i) {
    ...
    const ready =
      new Array(method.inputs.length)
        .fill()
        .map((k, j) => j)
        .find(j => !this.state.inputs[j]) === undefined;
    if (ready) {
      ...
      <Segment color={color}>
        ...
        <Header style={{ marginTop: '-1.5rem' }} as="h3">
          ...
          {method.signatureHash && (
            <code className="signature">{method.signatureHash}</code>
          )}
        </Header>
        {method.notice && (
          <ReactMarkdown containerTagName="h4" source={method.notice} />
        )}
