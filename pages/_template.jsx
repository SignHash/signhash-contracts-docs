...
export default class Index extends Component {
  constructor(props) {
    ...
    if (config.interaction && config.interaction.providerUrl) {
      ...
      this.web3.setProvider(
        new this.web3.providers.HttpProvider(config.interaction.providerUrl)
      );
    }
  }
  getChildContext() {
  ...
  render() {
    ...
    const docsRoute = this.props.route.childRoutes.find(
      route => route.path === prefixLink('/docs/')
    );
        ...
        <Container className="footer">
          <Segment secondary size="small" attached="top" compact>
            <Grid stackable>
              <Grid.Row>
                <Grid.Column width={6}>
                  <b>&copy; {config.author}</b> - {config.license},{' '}
                  {new Date(config.buildTime).getFullYear()}
                </Grid.Column>
                <Grid.Column width={10} textAlign="right">
                  Docs built using <b>Solidity {config.compiler}</b> on{' '}
                  <b>{new Date(config.buildTime).toLocaleDateString()}</b>
                </Grid.Column>
              </Grid.Row>
            </Grid>
          </Segment>
